...
records airbnb actually serves. You do that in two files, and every surface
updates itself.

## 1. Model the record

In `airbnb/airbnb.go`, add a struct for the thing you are fetching
...
type Item struct {
    ...
    Owner string `json:"owner" kit:"link,kind=airbnb/user"` // an edge to another record
}
...
- `kit:"link,kind=<scheme>/<type>"` marks an outbound edge. It can point at
  another airbnb type or at another site entirely, which is what lets a
  host walk the graph across tools.

## 2. Declare the operation

In `airbnb/domain.go`, add an input struct and a handler, then register
it in `Register`:
...
```bash
airbnb item <id>                 # the command
curl 'localhost:7777/v1/item/<id>'      # the route, under serve
ant get airbnb://item/<id>       # the URI dereference, via a host
```
