...
describes what users get. Pick a format with `-o`, or let airbnb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
airbnb <command> -o table   # aligned columns for reading
airbnb <command> -o jsonl   # one JSON object per line, for piping
airbnb <command> -o json    # a single JSON array
airbnb <command> -o csv     # spreadsheet friendly
airbnb <command> -o tsv     # tab-separated
airbnb <command> -o url     # just the URL column
airbnb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
airbnb <command> --fields id,title,url
```
...
```bash
airbnb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
airbnb <command>            # a table, because this is a terminal
airbnb <command> | wc -l    # JSONL, because this is a pipe
```
