...
// HTTP behaviour is covered in airbnb_test.go.

func TestDomainInfo(t *testing.T) {
	info := Domain{}.Info()
	if info.Scheme != "airbnb" {
		t.Errorf("Scheme = %q, want airbnb", info.Scheme)
	}
	if len(info.Hosts) == 0 || info.Hosts[0] != Host {
		t.Errorf("Hosts = %v, want [%s]", info.Hosts, Host)
	}
	if info.Identity.Binary != "airbnb" {
		t.Errorf("Identity.Binary = %q, want airbnb", info.Identity.Binary)
	}
}
...
func TestHostWiring(t *testing.T) {
	...
	if want := "airbnb://page/wiki/Go"; u.String() != want {
	...
	got, err := h.ResolveOn("airbnb", "about")
	if err != nil || got.String() != "airbnb://page/about" {
		t.Errorf("ResolveOn = (%q, %v), want airbnb://page/about", got.String(), err)
	}
}
